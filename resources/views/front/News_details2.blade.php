...
@section('title', __('news2.meta_title'))
@section('meta_description', __('news2.meta_description'))

@push('head')
  {{-- Open Graph --}}
  <meta property="og:title" content="{{ __('news2.og_title') }}">
  <meta property="og:description" content="{{ __('news2.og_description') }}">
    ...
    <div class="w-full bg-white shadow-sm border-b border-gray-200">
      ...
      <div class="flex flex-col gap-10 items-center py-14">
        <nav class="breadcrumb flex items-center justify-center gap-3 text-sm text-cp-light-grey" aria-label="Breadcrumb" data-reveal="fade-up">
          <a href="{{ route('front.index') }}" class="hover:text-cp-dark-blue">{{ __('news2.breadcrumb_home') }}</a>
          <span>/</span>
          <a href="{{ route('front.news') }}" class="hover:text-cp-dark-blue">{{ __('news2.breadcrumb_news') }}</a>
          <span>/</span>
          <span class="font-semibold text-cp-black">{{ __('news2.breadcrumb_details') }}</span>
        </nav>
      </div>
    </div>
  </div>
  ...
  <div class="container mx-auto max-w-6xl px-4 py-12 grid lg:grid-cols-3 gap-10">
    ...
    <article class="lg:col-span-2" itemscope itemtype="https://schema.org/NewsArticle">
      ...
      <div class="mt-3 text-xs text-slate-500 flex flex-wrap items-center gap-4" data-reveal="fade-up">
        <span itemprop="author" itemscope itemtype="https://schema.org/Person">
          👤 <span itemprop="name">{{ __('news2.author') }}</span>
        </span>
        <time itemprop="datePublished" datetime="2025-04-09">{{ __('news2.date_display') }}</time>
        <meta itemprop="dateModified" content="2025-04-09">
        <span>{{ __('news2.read_time') }}</span>
        <a href="#comments" class="hover:text-cp-dark-blue">{{ __('news2.comments_count') }}</a>
      </div>
      ...
      <div class="prose max-w-none mt-6 prose-p:leading-relaxed prose-img:rounded-xl" data-reveal="fade-up">
        ...
        <h3 class="font-bold">{{ __('news2.how_title') }}</h3>
        <ul class="list-disc pl-6">
          <li>{!! __('news2.how_1_html') !!}</li>
          <li>{{ __('news2.how_2') }}</li>
          <li>{{ __('news2.how_3') }}</li>
          <li>{{ __('news2.how_4') }}</li>
        </ul>
        ...
        <h3 class="font-bold">{{ __('news2.benefits_title') }}</h3>
        <ul class="list-disc pl-6">
          <li>{{ __('news2.benefit_1') }}</li>
          <li>{{ __('news2.benefit_2') }}</li>
          <li>{{ __('news2.benefit_3') }}</li>
          <li>{{ __('news2.benefit_4') }}</li>
        </ul>
      ...
      <div class="mt-8 flex flex-wrap items-center gap-4 justify-between border-y py-4" data-reveal="fade-up">
        <div class="flex flex-wrap gap-2">
          <span class="text-sm text-slate-500">{{ __('news2.tags') }}</span>
          @foreach(__('news2.tags_list') as $tag)
            <a href="#" class="px-2 py-1 rounded border text-sm hover:bg-slate-50">{{ $tag }}</a>
          @endforeach
        </div>
        <div class="flex items-center gap-2 text-sm">
          <span class="text-slate-500">{{ __('news2.share') }}</span>
      ...
      <div class="mt-8 flex items-center gap-4 p-5 rounded-xl border border-[#E8EAF2] bg-white" data-reveal="fade-up">
        ...
        <div>
          <div class="font-semibold">{{ __('news2.author') }}</div>
          <p class="text-sm text-slate-500">{{ __('news2.author_desc') }}</p>
        </div>
      </div>

      {{-- COMMENTS --}}
      <div id="comments" class="mt-10" data-reveal="fade-up">
        <div class="text-lg font-semibold mb-4">{{ __('news2.comments_title') }}</div>
        <div class="space-y-6">
          <div class="flex gap-3">
            <img src="{{ asset('assets/photos/photo5.png') }}" class="w-12 h-12 rounded-full object-cover" alt="User">
            <div>
              <div class="font-medium">
                {{ __('news2.sample_comment_name') }}
                <a href="#" class="text-cp-dark-blue text-sm ml-2">{{ __('news2.reply') }}</a>
              </div>
              <div class="text-xs text-slate-500">{{ __('news2.sample_comment_date') }}</div>
              <p class="mt-2">{{ __('news2.sample_comment_text') }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="mt-10" data-reveal="fade-up">
        <div class="text-lg font-semibold mb-4">{{ __('news2.leave_comment') }}</div>
        <form class="space-y-4">
          <div class="grid md:grid-cols-2 gap-4">
            <input class="w-full rounded-xl border border-[#E8EAF2] px-3 py-3" placeholder="{{ __('news2.form_name') }}">
            <input class="w-full rounded-xl border border-[#E8EAF2] px-3 py-3" placeholder="{{ __('news2.form_email') }}">
          </div>
          <input class="w-full rounded-xl border border-[#E8EAF2] px-3 py-3" placeholder="{{ __('news2.form_website') }}">
          <textarea class="w-full rounded-xl border border-[#E8EAF2] px-3 py-3" rows="5" placeholder="{{ __('news2.form_comment') }}"></textarea>
    ...
    <aside class="lg:col-span-1 space-y-8" aria-label="Sidebar">
      ...
      <div class="border rounded-xl p-6 bg-white" data-reveal="fade-left">
        <div class="font-semibold mb-4">{{ __('news2.recent_news') }}</div>
        <ul class="space-y-5 text-sm">
          <li>
            <a href="{{ route('front.news_details1') }}" class="group block">
              <img src="{{ asset('assets/news/news1.jpg') }}" alt="{{ __('news2.recent1_alt') }}"
                   class="w-full rounded-lg mb-2 aspect-[4/3] object-cover border border-[#E8EAF2]">
              <div class="font-medium group-hover:text-cp-dark-blue transition">{{ __('news2.recent1_title') }}</div>
              <div class="text-xs text-slate-500">👤 {{ __('news2.recent1_author') }} • {{ __('news2.recent1_date') }}</div>
            </a>
          </li>
          <li>
            <a href="{{ route('front.news_details3') }}" class="group block">
              <img src="{{ asset('assets/news/news3.jpg') }}" alt="{{ __('news2.recent3_alt') }}"
                   class="w-full rounded-lg mb-2 aspect-[4/3] object-cover border border-[#E8EAF2]">
              <div class="font-medium group-hover:text-cp-dark-blue transition">{{ __('news2.recent3_title') }}</div>
              <div class="text-xs text-slate-500">👤 {{ __('news2.recent3_author') }} • {{ __('news2.recent3_date') }}</div>
            </a>
          </li>
        </ul>
      </div>
      ...
      {{-- Tags --}}
      <div class="border rounded-xl p-6 bg-white" data-reveal="fade-left">
        <div class="font-semibold mb-3">{{ __('news2.tags') }}</div>
        <div class="flex flex-wrap gap-2">
          @foreach(__('news2.tags_list') as $tag)
